    ...
    <>
      <Link href={`/blog/${blog.id}`}>
        <div className=" rounded-lg p-4 bg-white dark:bg-gray-900 h-72 flex flex-col justify-around">
          ...
          <p className='text-sm '>{blog.body.length > 100 ? blog.body.substring(0, 200) + '...' : blog.body}</p>


          <div className='flex gap-3'>
            {blog.tags.map((tag, index) => (
              <span key={index} className=" px-4 py-2 border border-[#b32929] rounded  dark:bg-gray-700 dark:text-white">#{tag}</span>
            ))}
          </div>
